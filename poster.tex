...
The work by the researchers on the paper `Scalable Quantum Simulation of Molecular Energies' uses qubits kept at very low temperatures to run algorithms to find out about the fundamental properties of molecules.
The variational quantum eigensolver (VQE) and the phase estimation algorithm (PEA) are two types of algorithm which are performed using the qubits to find molecular energies. A special type of qubit known as an Xmon is used to run the algorithms. This type of qubit has a superposition of ``charge" states and can be used in quantum circuits.
